...
const FoundItemCard = ({ item }) => {
    ...
    const coverImage = images[0]?.url || fallbackImage;
    ...
    return (
        <article className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-500 hover:-translate-y-2 hover:border-emerald-300 hover:shadow-2xl hover:shadow-emerald-100">
            <div className="absolute inset-x-0 top-0 h-1 bg-linear-to-r from-emerald-400 via-cyan-400 to-sky-500 opacity-80" />
            ...
            <div className="relative h-56 overflow-hidden bg-slate-100">
                ...
                />
                <div className="absolute inset-0 bg-linear-to-t from-slate-950/65 via-slate-950/10 to-transparent opacity-80 transition-opacity duration-500 group-hover:opacity-95" />
                    ...
                    <div className="absolute bottom-4 right-4 flex -space-x-2">
                        {previewImages.map((image, index) => (
                            <img
                                ...
                                src={image.url}
                                alt={`${itemName} preview ${index + 2}`}
                                className="h-11 w-11 rounded-xl border-2 border-white object-cover shadow-md transition-transform duration-300 group-hover:-translate-y-1"
                            />
                        ))}
                        {images.length > 3 && (
                            <span className="grid h-11 w-11 place-items-center rounded-xl border-2 border-white bg-slate-950/80 text-xs font-bold text-white shadow-md backdrop-blur">
                                +{images.length - 3}
                            </span>
                        )}
                    </div>
